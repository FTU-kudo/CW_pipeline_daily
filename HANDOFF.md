...
## 6. Sửa lỗi Timeout do dữ liệu CW quá khứ & Bypass KBS (21/08/2026)
*Thời gian thực hiện: 21/08/2026 (Hoàn tất lúc 01:10 ICT)*

**Triệu chứng:** Pipeline trên GitHub Actions liên tục báo `TIMEOUT CACB2301` ở nguồn VCI, đồng thời ghi nhận `KBS fallback FAIL`. Hậu quả là VCI circuit breaker bị kích hoạt, làm fail toàn bộ 1209 mã CW.

- **Nguyên nhân cốt lõi (Root Causes):**
  1. **Lỗi KBS không hỗ trợ CW:** Khi fetch CW từ KBS, API luôn ném ra `ValueError`. Tuy nhiên, lỗi này trước đó bị tính nhầm vào `_kbs_circuit['fails']`, khiến circuit của KBS tự động bật sai cách.
  2. **Lỗi VCI Timeout cho CW đã hết hạn:** Các mã như `CACB2301` đã đáo hạn từ 08/2023. Pipeline cũ cố gắng lấy dữ liệu từ `2023-08-04` đến `2026-08-22`. Việc yêu cầu một khoảng dữ liệu 3 năm không tồn tại khiến server VCI bị kẹt và gây ra Timeout 35s trên GitHub Actions runner.

- **Các Fix đã triển khai trong `pipeline.py`:**
  1. **Bypass KBS hoàn toàn cho CW:** Bổ sung regex nhận diện symbol CW (VD: `^C[A-Z]{2,4}\d{4}$`). Nếu là CW, pipeline sẽ không bao giờ gọi KBS để tránh `ValueError` lặp lại.
  2. **Bỏ qua fetch OHLCV cho CW hết hạn:** Xây dựng `expired_ldt_map` từ cột `ngay_gd_cuoi_cung` của Vietstock metadata. Nếu một CW đã hết hạn và cache đã chứa đủ dữ liệu đến trước ngày đáo hạn, hệ thống sẽ in log `Skip (het han)` và **không gửi API request tới VCI**. 
  3. **Bảo toàn dữ liệu lịch sử:** Các CW bị skip (như `CACB2301`) vẫn được ghép nối với dữ liệu mới để đưa ra bảng `cw_master` cuối cùng, đáp ứng 100% nhu cầu phân tích dữ liệu quá khứ từ 2023.

---

## 7. Bảo mật cấu hình & Xóa HANDOFF.md khỏi Repo (21/08/2026)
*Thời gian thực hiện: 21/08/2026 (Hoàn tất lúc 01:40 ICT)*

- **Vấn đề:** File `HANDOFF.md` chứa chi tiết luồng xử lý và nhật ký công việc riêng tư (bí mật), không nên được publish công khai lên GitHub.
- **Giải pháp:** 
  - Đã thực hiện `git rm --cached HANDOFF.md` để gỡ file khỏi lịch sử theo dõi (tracking) của Git trên server.
  - Cập nhật `.gitignore` để ẩn hoàn toàn file `HANDOFF.md` ở những lần push sau.
  - File vẫn tồn tại 100% trên máy local của người dùng để đọc và lưu trữ cá nhân, nhưng hoàn toàn tàng hình đối với repo công khai.
